...
func (rc *Client) GetClient() r.UniversalClient {
	return rc.Cli
}

func (rc *Client) GetContext() context.Context {
	return rc.Ctx
}
...
func (rc *Client) MGet(keys ...string) ([]string, error) {
	...
	if err := sliceCmd.Err(); err != nil && err != r.Nil {
		return []string{}, err
	}
	tmp := sliceCmd.Val()
...
func (rc *Client) Set(key string, value interface{}, expire time.Duration) bool {
	var err error
...
func (rc *Client) HGetAll(key string) map[string]string {
	...
	if err := stringMapCmd.Err(); err != nil && err != r.Nil {
...
// HMSet 设置redis的hash
func (rc *Client) HMSet(key string, hash map[string]interface{}, expire time.Duration) bool {
	if len(hash) > 0 {
		var err error
